...
#define TFT_RST   -1
#define TFT_DC     9
#define TFT_ROTATION 1 // (4 + 1)

#include "LGFX_Spresense_ILI9486.hpp"
LGFX tft;

#define USE_TP_ILI9486
...
void CamCB(CamImage img)
{
  ...
  if (img.isAvailable()) {
      ...
      if (err != CAM_ERR_SUCCESS)
        Serial.printf("convertPixFormat = %u\n", err);
      ...
      tft.pushImage(80, 5, img.getWidth(), img.getHeight(), (uint16_t *)img.getImgBuff());
    //tft.pushImage((tft.width() - img.getWidth()) / 2, (tft.height() - img.getHeight()) / 2,
    //               img.getWidth(), img.getHeight(), (uint16_t *)img.getImgBuff());
...
void setup()
{
  ...
  tft.fillRect( 0, 250, 480, 2, TFT_DARKGREY);
 #endif
#endif
...
String inputString()
{
  ...
  do {
#ifdef USE_ILI9486
 #define ZONE_3U(x, y) ((x > 400) && (y < 140))        // ZONE_3U
 #ifdef USE_TP_ILI9486
    uint16_t px, py;
    if ( getTouch(&px, &py) && (ZONE_3U(px, py)) ) {
      input = "P";
      break;
    }
 #endif
...
#ifdef USE_ILI9486
 // Thumbnail
 #define THUMB_PX(n)  (30 + (n % 5) * 84)
 #define THUMB_PY()   (250 + 5)
    static size_t spos = -1;
    if (spos >= 0)
      tft.drawRect(THUMB_PX(spos++), THUMB_PY(), 82, 62, TFT_DARKGRAY );
    tft.fillRect  (THUMB_PX(spos  ), THUMB_PY(), 82, 62, TFT_DARKGRAY );
    tft.drawRect  (THUMB_PX(spos  ), THUMB_PY(), 82, 62, TFT_LIGHTGREY);
 #if (1)
    tft.setSwapBytes(true);
    size_t reDrawH = ( 60 - ( img.getHeight() * 80 / img.getWidth() ) ) / 2;
    tft.drawJpg(img.getImgBuff(), img.getImgSize(), THUMB_PX(spos) + 1, THUMB_PY() + 1 + reDrawH, 80, 60, 0, 0, (float)(80 / img.getWidth()));
    tft.setSwapBytes(false);
 #endif
    tft.setTextSize(1);
    tft.setTextColor(TFT_WHITE, TFT_BLACK);
    tft.drawCenterString(filename + 4, THUMB_PX(spos) + (84 / 2), THUMB_PY() +  2, 1);
#endif
