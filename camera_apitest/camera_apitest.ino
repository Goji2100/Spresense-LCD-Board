...
//----
//#define USE_LIB_ADAFRUIT_ILI9341  // Adafruit_GFX
//----
//#define USE_ADAFRUIT_ILI9341      // LovyanGFX
//#define USE_STME610
//----
  #define USE_ILI9341               // LovyanGFX
//#define USE_ILI9486               // LovyanGFX
//#define USE_ILI9488               // LovyanGFX
  #define USE_XPT2048
//----

#if defined(USE_ILI9341)||defined(USE_ILI9486)||defined(USE_ILI9488)||defined(USE_ADAFRUIT_ILI9341) 
  #include "LGFX_Spresense_ILI9xxx.hpp"

  volatile bool shutterSW = false;
#endif
  ...
  Adafruit_ILI9341 tft = Adafruit_ILI9341(&SPI4, TFT_DC, TFT_CS, TFT_RST);
#endif
...
void CamCB(CamImage img)
{
  ...
  if (img.isAvailable()) {
      ...
      tft.pushImage(Z2_px, Z2_py, img.getWidth(), img.getHeight(), (uint16_t *)img.getImgBuff());
      tft.setSwapBytes(false);
      displayFPS(0);
      #if defined(USE_XPT2048)||defined(USE_STME610)
        if (shutterSW) getShutter(); else shutterSW = getShutter(); 
      #endif
    #endif
...
void setup()
{
  ...
  if (err != CAM_ERR_SUCCESS) {
    ...
    err = theCamera.begin(2, CAM_VIDEO_FPS_60, 240, 180, CAM_IMAGE_PIX_FMT_RGB565, 7);
  #else
    err = theCamera.begin(2, CAM_VIDEO_FPS_60, CAM_IMGSIZE_QVGA_H,  CAM_IMGSIZE_QVGA_V,  CAM_IMAGE_PIX_FMT_RGB565, 7);
    if (err != CAM_ERR_SUCCESS) {
      printError(err);
    }
  #endif
...
void menuTop()
{
  ...
  String input = inputString();
    ...
    CamErr err = theCamera.startStreaming(false);
    if (err != CAM_ERR_SUCCESS) {
      printError(err);
    }
  #endif
  ...
  if      (input == "I")    { menuInfo();        }
    ...
    if (input.length() > 2) {
      g_pict_id = input.substring(1).toInt();
    }
    takePicture();
  }  
  #else
